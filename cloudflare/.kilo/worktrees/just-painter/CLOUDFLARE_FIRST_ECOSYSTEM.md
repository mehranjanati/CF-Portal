...
- `AI inference` روی Workers AI و `AI Gateway` باشد
...
| `Model access` | provider-specific | `Workers AI` + `AI Gateway` | مهاجرت مستقیم |
...
## 5.1 Target Architecture V2: Cloudflare-Only / TS+Rust / VibeSDK-First
...
3. **هر چیز user-facing روی Workers/Agents اجرا می‌شود**
4. **هر چیز publishable روی Workers for Platforms می‌نشیند**
5. **هر چیز white-label با Cloudflare for SaaS تحویل می‌شود**
6. **هیچ state حیاتی خارج از Cloudflare نگه‌داری نمی‌شود**
...
هدف Phase 1 این نیست که همه featureها کامل شوند؛ هدف این است که **اسکلت production-grade پلتفرم Cloudflare-only** ساخته شود.
